...
function decodeCredentialInfo(credentialInfo) {
  const clientDataJSON = bufferToString(credentialInfo.response.clientDataJSON);
  const attestationObject = CBOR.decode(
    credentialInfo.response.attestationObject
  );
  ...
  const pubKey = CBOR.decode(authData.COSEPublicKey.buffer);
